...
function AllSiteIncident({siteIncidentData}) {
    ...
    const categories = ['Shelter', 'Security', 'Others'];
...
// Generate years array with future years
    ...
    const filterReports = (data) => {
        return data.filter(report => {
            ...
            const reportCluster = report.siteInfo?.cluster;
            const reportCategory = report.siteIncidentInfo?.category;
    ...
    const exportToExcel = () => {
        const filteredData = filterReports(siteIncidentData);
        ...
        const workbookData = filteredData.map(report => ({
            ...
            // Site Information
            'Site ID': report.siteInfo?.siteId || 'N/A',
            'State': report.siteInfo?.state || 'N/A',
            'Cluster': report.siteInfo?.cluster || 'N/A',
            'Location': report.siteInfo?.location || 'N/A',
            'Site Type': report.siteInfo?.type || 'N/A',

            // Site Incident Information
            'Category': report.siteIncidentInfo?.category || 'N/A',
            'Sub-Category Details': (() => {
                const info = report.siteIncidentInfo?.subCategory;
                if (!info) return 'N/A';
                if (info.shelter) return `Shelter - ${info.shelter}`;
                if (info.security) return `Security - ${info.security}`;
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Site Incidents');
        ...
        const filename = `site_incidents${filters.length ? '_' + filters.join('_') : ''}_${new Date().toISOString().split('T')[0]}.xlsx`;
