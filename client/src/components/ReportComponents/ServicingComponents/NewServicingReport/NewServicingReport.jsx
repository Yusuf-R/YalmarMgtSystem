...
function NewServicingReport({allStaff, allSite}) {
    ...
    const Clear = () => {
        ...
        setShelterStructure('');
        setPmType('');
        setServicingDate(null);
        setGenModes('');
        setDefaultOperation('');
        setGen1Type('');
        setGen1Display('');
        setGen1WorkingStatus('');
        setGen2Type('');
        setGen2Display('');
        setGen2WorkingStatus('');
        setAcInstalled('');
        setNoOfAcInstalled('');
        setAc1Status('');
        setAc2Status('');
        setSiteCleaningStatus('');
        setShelterCleaningStatus('');
        setAwlWorkingStatus('');
        setSecurityLightAvailability('');
        setSecurityLightStatus('');
        setFloodLightAvailability('');
        setFloodLightStatus('');
        setSecurityStatus('');
        setSiteAccess('');
    };
    ...
    const handleGen2Hr = (event) => {
        event.preventDefault();
        setGen2Hr(event.target.value);
        if (Gen2Hr !== 'Enter Value') {
    ...
    // use Effect section for manging conditional data
    //genModes
    useEffect(() => {
        if (genModes === 'GEN-1') {
    ...
    // if errors contains any error then display the error
    ...
    const SubmitData = async (data) => {
        try {
            await newServiceReportSchema.validate(data, {abortEarly: false});
            ...
            if (data.generatorPM.gen1Hr === 'Enter Value' && data.generatorPM.customGen1Hr) {
                data.generatorPM.gen1Hr = Number(data.generatorPM.customGen1Hr);
                delete data.generatorPM.customGen1Hr;
            }
            if (data.generatorPM.gen2Hr === 'Enter Value' && data.generatorPM.customGen2Hr === 0) {
                data.generatorPM.gen2Hr = Number(data.generatorPM.customGen2Hr);
                delete data.generatorPM.customGen2Hr;
            }
            if (data.generatorPM.gen2Hr === 'NOT-APPLICABLE') {
                delete data.generatorPM.customGen2Hr;
            }
            if (data.type) {
                delete data.type;
            ...
            mutation.mutate(data, {
                onSuccess: (response) => {
                    if (response) {
                        toast.success('Servicing Record created successfully');
                        ...
                        toast.error('Failed to create new service record');
                    }
                },
                onError: (error) => {
