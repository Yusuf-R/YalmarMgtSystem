...
function AllLeaveRequest({leaveReqData}) {
    ...
    useEffect(() => {
        if (pathname.startsWith("new")) {
            setActiveTab("/dashboard/admin/settings/leave-request/new");
        } else if (pathname.startsWith("/dashboard/admin/settings/leave-request")) {
            setActiveTab("/dashboard/admin/settings/leave-request");
        } else if (pathname.startsWith("/dashboard/admin/settings/biodata")) {
    ...
    const table = useMaterialReactTable({
        ...
        renderRowActions: ({row}) => {
            ...
            const reqAction = async () => {
                ...
                const encryptedReqData = await AdminUtilities.encryptData(reqData);
                // Set the encrypted data in Zustand store
                setEncryptedStaffLeaveRequestData(encryptedReqData, encryptedReqId);
                router.push(`/dashboard/admin/settings/leave-request/view`);
            };
            return (
                <>
                    <Stack direction='row'>
                ...
                <Stack direction="row" spacing={2} sx={{justifyContent: "flex-start"}}>
                    ...
                    >
                        {[
                            ...
                            {label: "BioData", value: "/dashboard/admin/settings/biodata"},
